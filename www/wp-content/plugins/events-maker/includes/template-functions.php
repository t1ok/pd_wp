...
if ( ! function_exists( 'em_display_event_categories' ) ) {
	...
	function em_display_event_categories( $post_id = 0 ) {
		...
		$categories = get_the_term_list( $post_id, 'event-category', __( '<span class="label label-warning">Category</span> ', 'events-maker' ), ', ', '' );
...
if ( ! function_exists( 'em_display_event_tags' ) ) {
	...
	function em_display_event_tags( $post_id = 0 ) {
		...
		$tags = get_the_term_list( $post_id, 'event-tag', __( '<span class="label label-warning">Tags</span> ', 'events-maker' ), ', ', '' );
...
if ( ! function_exists( 'em_display_event_locations' ) ) {
	...
	function em_display_event_locations( $post_id = 0 ) {
		...
		<?php $output = get_the_term_list( $post_id, 'event-location', __( '<span class="label label-warning">Location</span> ', 'events-maker' ), ', ', '' ); ?>
		...
		<div class="entry-meta">
			...
			<?php if ( is_single() ) : ?>
				...
				<?php if ( isset( $event_display_options['display_location_details'] ) && $event_display_options['display_location_details'] == true ) : ?>
					...
					<?php
					$output = __( '<span class="label label-warning">Location</span> ', 'events-maker' );
...
if ( ! function_exists( 'em_display_event_organizers' ) ) {
	...
	function em_display_event_organizers( $post_id = 0 ) {
		...
		$output = get_the_term_list( $post_id, 'event-organizer', __( '<span class="label label-warning">Organizer</span> ', 'events-maker' ), ', ', '' );
		?>
		...
		<?php if ( is_single() ) : ?>
			...
			<?php if ( isset( $event_display_options['display_organizer_details'] ) && $event_display_options['display_organizer_details'] == true ) : ?>
						...
						<?php
						$output = __( '<span class="label label-warning">Organizer</span> ', 'events-maker' );
...
if ( ! function_exists( 'em_page_title' ) ) {
	...
	function em_page_title( $echo = true ) {
		...
		elseif ( is_tax( 'event-category' ) )
			$page_title = sprintf( __( 'Події Категорії: %s', 'events-maker' ), single_term_title( '', false ) );
		elseif ( is_tax( 'event-location' ) )
			$page_title = sprintf( __( 'Події Локації: %s', 'events-maker' ), single_term_title( '', false ) );
		elseif ( is_tax( 'event-organizer' ) )
			$page_title = sprintf( __( 'Події Організатори: %s', 'events-maker' ), single_term_title( '', false ) );
		elseif ( is_tax( 'event-tag' ) )
			$page_title = sprintf( __( 'Події Мітки: %s', 'events-maker' ), single_term_title( '', false ) );
		else
			$page_title = __( 'Події', 'events-maker' );
